...
var _nav_axis_locks: Dictionary = {}         # {pi: bool}
...
func _get_confirmed_character_owner(char_index: int, ignored_player: int = -1) -> int:
	for pi: int in _player_confirmed:
...
func _draw_character_index(font: Font, origin: Vector2, current_index: int) -> void:
	...
	for i in _characters.size():
		...
		var chip_color: Color = char_data.get("color", Color.WHITE) as Color
		var chip_name: String = char_data.get("name", "") as String
...
func _draw() -> void:
	...
	for i in SLOT_COUNT:
		...
		var char_index: int = _player_cursor[player_index] as int
		var char_data: Dictionary = _characters[char_index]
		...
		var taken_by: int = _get_confirmed_character_owner(char_index, player_index)
		var is_taken: bool = confirmed
		...
		draw_rect(Rect2(card_rect.position, Vector2(card_rect.size.x, 7.0 if confirmed else 5.0)),
			Color(char_color, 1.0 if confirmed else 0.92))
		_draw_character_index(font, Vector2(card_x + CARD_MARGIN, card_y + 12.0), char_index)
		_draw_selection_badge(font,
			Rect2(card_x + card_w - CARD_MARGIN - 170.0, card_y + 12.0, 170.0, 24.0),
			_player_display_name(player_index), char_color, confirmed)
		_draw_side_arrows(font, card_rect, char_color)
		...
		if confirmed:
			_draw_selection_badge(font,
				...
				"%s ELIGIÓ" % _player_display_name(player_index), char_color, true)
		elif taken_by >= 0:
			_draw_selection_badge(font,
			...
			_draw_selection_badge(font,
				Rect2(card_x + CARD_MARGIN, card_y + card_h - 40.0, card_w - CARD_MARGIN * 2.0, 26.0),
				"%s ELIGIENDO" % _player_display_name(player_index), char_color, false)
		continue
	...
	for pi: int in _viewer_cursor:
		...
		var label := "%s: %s %s" % [_player_display_name(pi), cname, status_mark]
		var lw := font.get_string_size(label, HORIZONTAL_ALIGNMENT_LEFT, -1, 14).x
		draw_string(font, Vector2(cx - lw / 2.0, status_y),
			label, HORIZONTAL_ALIGNMENT_LEFT, -1, 14,
